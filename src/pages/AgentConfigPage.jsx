...
const API_BASE_URL = import.meta.env.VITE_BLOGS_API_URL || "http://localhost:8000";
...
const AgentConfigPage = () => {
  ...
  const handleAISuggestion = async (suggestionType, currentValues) => {
    ...
    try {
      ...
      // Include the live, unsaved form data so the AI has maximum context
      const payload = {
        profile_id: activeProfileId,
        current_values: currentValues,
        limit: 5,
        industries: formData.target_industries, // Live state
        tech_focus: formData.technology_focus   // Live state
      };
      ...
      if (response.ok) {
        ...
        // Use FastAPI's specific error detail if available
        setError(result.detail || `Failed to generate ${suggestionType} suggestions`);
      }
    } catch (err) {
